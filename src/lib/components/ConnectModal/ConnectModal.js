...
export default function ConnectModal({ lit = false, title = "Connect to join the discussion", description = "You must be connected to share posts or reactions.", hide }) {
  ...
  return(
    <div class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="fixed inset-0 cursor-pointe blur-lgr" style={{background: theme?.bg?.secondary ? theme.bg.secondary : defaultTheme.bg.secondary, opacity: 0.80 }}></div>

      <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <div class="relative transform overflow-hidden rounded-lg px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6" ref={wrapperRef} style={{background: theme?.bg?.main ? theme.bg.main : defaultTheme.bg.main }}>
            <div>
              <div class="mt-3 text-center sm:mt-5">
                <h3 class="text-lg font-medium leading-6" style={{ color: getThemeValue("color", theme, "main") }} id="modal-title">{title}</h3>

                {/** Show description only if available */}
                {description &&
                  <div class="mt-1 mb-2">
                    <p class="text-base" style={{ color: getThemeValue("color", theme, "secondary") }}>{description}</p>
                  </div>
                }
              </div>
            </div>
